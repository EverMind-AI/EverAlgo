# EverCore Architecture Decision Records (ADR)

本目录收录 EverCore 设计中的关键架构决策。每个 ADR 独立讨论一个决策，包含完整的优劣分析、对 EverCore 的适配度评估、决策结论与行业实证印证。
...
| [003](003-namespace-package-pep420.md) | namespace 模式 | ✅ Accepted | PEP 420 共享 `evercore.*` namespace（B 风格）|
| [004](004-providers-nested-in-llm.md) | LLM provider 物理位置 | ✅ Accepted | 内嵌于 `llm/providers/` 子包 |
| [005](005-testing-as-public-subpackage.md) | testing 暴露形态 | ✅ Accepted | 公开子包 `evercore.testing`，`assertions + fake_llm` 两件套 |
| [006](006-clustering-independent-subpackage.md) | clustering 形态 | ✅ Accepted | 独立工具性子包 `evercore-clustering` |
...
| [012](012-llm-stack-architecture.md) | LLM 抽象层架构 | ✅ Accepted | 各家原生 SDK + Protocol + 双层路由（Scene 出 EverCore / Provider 在 EverCore） + LLMError 7 子类混合多重继承 + 算法层不加 retry |
...
3. **候选方案**：列出全部候选（不预设倾向）
4. **客观优劣分析**：每个方案的普世优劣（不带 EverCore 滤镜）
5. **对 EverCore 适配度评估**：逐条优势/劣势对 EverCore 的影响（强需要 / 不在意 / 强烈介意 / 可 mitigate / 用不上）
...
- **追溯 EverCore 硬约束**：H1 EverOS 文档契约 / H2 升级 A 不动 B / H3 算法同学迭代速度 / H4 无状态接口 / H5 跨包紧密联动 / H6 v0.x 演化阶段 / H7 可识别系列前缀
