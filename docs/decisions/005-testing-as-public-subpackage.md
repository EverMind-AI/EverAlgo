# ADR 005: testing 暴露形态 — 公开子包 `everalgo.testing`，assertions + fake_llm 两件套
...
相关硬约束：
- **H3** 算法同学迭代速度（写单测要顺手）
- 算法库 vs SDK 分类（[ADR 008](008-re-export-vs-client-facade.md)）：EverAlgo 是算法库

## 候选方案

| 方案 | 描述 |
|------|------|
| **A. 公开子包 `everalgo.testing`，assertions + fake_llm 两件套** | `everalgo-core` dist 内含 `everalgo/testing/` 子包；`assertions.py` 提供领域专用断言（`assert_memcell_equal` 等），`fake_llm.py` 提供可编程 fake LLM 客户端；下游 `from everalgo.testing import ...` 直接用 |
| B. 同 A 但加 `fixtures/` 子目录 | A + `everalgo/testing/fixtures/` 含预设样本（sample_messages / sample_memcell 等）|
| C. 独立 distribution `everalgo-testing` | 拆出 testing 成独立 dist，dev-only 安装 |
| D. 不暴露 testing 子包 | 下游自己 mock，不提供官方测试辅助 |

## 客观优劣分析

### A. 公开 `everalgo.testing`，两件套 优势

| 维度 | 说明 |
|------|------|
| 下游写单测低门槛 | `from everalgo.testing import fake_llm` 一行即用 |
...
### A. 公开 `everalgo.testing`，两件套 劣势

| 维度 | 说明 |
|------|------|
| `everalgo-core` dist 体积 +少量 | testing 子包代码增加 dist 体积（实际几 KB，可忽略）|
| 生产环境也装了 testing | 用户生产部署时 `everalgo-core` 内含 testing 子包（实际不会 import 不影响）|
...
### C. 独立 dist `everalgo-testing` 优势/劣势
...
- 用户多一个 `pip install everalgo-testing` 步骤，门槛上升
...
**不选 B**——预设样本若未来必要，独立 `everalgo.examples` 子包按 sklearn.datasets 模式承接，不混入 testing。
...
**选 A：公开子包 `everalgo.testing`，assertions + fake_llm 两件套**。

`everalgo.testing/` 内容：

| 子模块 | 内容 |
|--------|------|
| `everalgo.testing.assertions` | `assert_memcell_equal` / `assert_episode_equal` / `assert_rank_output_close(top_k, atol)` 等针对 everalgo 数据类型的断言函数 |
| `everalgo.testing.fake_llm` | 可编程 fake LLM 客户端（按 scene / prompt 模板返回预设响应），下游 monkeypatch 替换真实 LLM |

**显式不放**：
- 预设样本数据（fixtures / samples）
- pytest plugin / conftest 共享

未来若需预设样本，独立 `everalgo.examples` 子包（仿 `sklearn.datasets`），不混入 testing。
...
| **langchain-tests** | 测试基础设施 / utils | 独立 `langchain-tests` dist（langchain 是少数选 C 的，但 EverAlgo 适配度低）|

**关键**：numpy / pandas / pytorch / sklearn **都不放预设样本到 testing 子包**。预设样本独立 namespace（如 `sklearn.datasets`）是更主流模式。

EverAlgo 选 A + 拒绝 fixtures，与 numpy / pandas / pytorch 路线一致。
...
- [ADR 002 多 distribution](002-multi-distribution-vs-single.md) — testing 并入 everalgo-core 的依据
