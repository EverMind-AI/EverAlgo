...
EverAlgo 算法库需调 LLM。多个交叉决策需统一沉淀（之前散落 v0.29 / v0.30 changelog）：
...
| **Protocol structural** | EverAlgo 选择，[ADR 011](011-protocol-vs-abc.md) 已论证 |
...
| 仅 Provider 路由 | EverAlgo 不做 scene，仅做 Provider 适配 |
...
| **B 精简 + 混合多重继承** | LLMError + 7 子类 + 让原生 SDK catch 仍有效 | EverAlgo 选择（Letta + LangChain partner 启发） |
...
| **B 同款** | EverAlgo 选择 | LangChain Core 同款 |
...
- 多 SDK 错误 schema 不一致，需 EverAlgo 内部归一
- 多 SDK 安装依赖
...
| P2 错误层级完备 | ✅ 受益（EverAlgo 归一基础）|
| P2 多 SDK 错误归一成本 | ⚠️ 可 mitigate（adapter 出口集中映射）|
| P3 LiteLLM 100+ provider | ⚠️ 用不上（EverAlgo 主用 OpenAI / Anthropic / Bedrock 即可，OpenAI compat 已覆盖大部分）|
...
B 派 4/4 ABC 真实驱动：concrete mixin（LangChain ~40+ / LlamaIndex ~32 / Letta 11）+ 多继承胶水（`BaseComponent` / `RunnableSerializable` / `ComponentBase`）+ 实例化早失败 + 生态级 framework 需求。EverAlgo **一条都不命中**——是生态级 framework 才有的需求，library 级别工具无此压力。维持 [ADR 011](011-protocol-vs-abc.md) Protocol。
...
- 7 子类按 EverAlgo 实际处置策略（重试 / 不重试 / 裁剪输入重试）划分
...
→ **EverAlgo opensource 算法层不加 retry**（LangChain Core 同款）。高可用编排（multi-key 轮转 / 跨 Provider fallback / 长时间退避 / 配额降级 / 租户级路由）**不属于开源版需求**——这些是部署侧 reliability 关注点，不影响算法本身 SOTA 复现，开源版不做。`LLMClient` Protocol 天然支持装饰器扩展，部署方有需要时可自行实现透明叠加。
...
- `everalgo/llm/providers/{openai_compat, anthropic, bedrock}.py` 各自薄适配
...
→ EverAlgo **一条不命中**（算法库无状态 + 不挂体系 + 不是 partner 生态 + library 不是 framework），Protocol 维持稳健。
...
| 现状 | 迁移到 EverAlgo |
|------|---------------|
| `OpenAIProvider`（aiohttp 自实现 5 次重试）| `everalgo.llm.providers.openai_compat`（`openai.AsyncOpenAI` 薄 wrap，**单 api_key**）|
...
- [ADR 003 PEP 420 namespace](003-namespace-package-pep420.md) — `everalgo.llm` 子包遵循 namespace 规范
- [ADR 004 providers 内嵌于 llm/](004-providers-nested-in-llm.md) — providers in-tree 不用 partner pip 包
- [ADR 008 re-export facade](008-re-export-vs-client-facade.md) — `everalgo.configure(llm=...)` 全局注入而非 Client 类
