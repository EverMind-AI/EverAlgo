...
> 给算法同学和新同学的 onboarding 文档。读完应该能够回答：EverCore 是什么、负责什么、不负责什么、怎么调用、命名/调用契约是什么。
>
...
## 1. 鸟瞰：EverOS 依赖 EverCore

**EverOS 是 AI 记忆管理系统**，对外提供完整产品能力（API、持久化、编排、记忆生命周期管理）。**EverCore 是算法库**，被 EverOS 依赖来实现 Extract / Rank 算法 IP。两者是"产品 ↔ 依赖库"关系：
...
**EverCore** —— EverOS 依赖的算法库。**无状态、不碰 DB、不读写文件系统、不持有业务编排概念**。输入输出全部是 in-memory 数据结构，类比 EverOS 之于 EverCore 就像 FastAPI 应用之于 numpy / sklearn——产品负责一切，库只负责算法。

边界用一句话讲清：

- EverCore 不知道有 DB，由 caller 把数据备好传入
- EverCore 不知道"哪个场景用哪个 LLM"，由 caller 选好 client 传入
- EverCore 不知道并发和分布式锁，由 caller 自行串行化 read-modify-write

---

## 2. 算法库定位

EverCore 与 sklearn / pytorch / DSPy 同阵营，**不是** LangChain / LlamaIndex 那种端到端框架：

| 维度 | 算法库（EverCore 阵营）| 端到端框架（LangChain 阵营）|
|------|---------------------|--------------------------|
...
EverCore **提供**：算法 IP（Extract + Rank 双主轴的算子）、LLM 调用门面、Provider 路由、prompt validator、testing 辅助。

EverCore **不提供**：API 网关、持久化、并发编排、retry / fallback、多 key 轮转、多租户、配额、observability。这些归工程载体（EverOS）。
...
EverCore 的算法职责分两条主轴，对外契约对称（**无状态、不碰 DB、输入输出都是内存数据结构**）：
...
```python
import asyncio
from evercore import user_memory, rank
...
| 基础设施 | `llm` / `prompts` / `types` / `testing` | 全部在 `evercore-core` distribution |

**两条 import 路径同时有效**——按你的角色选：

- **EverOS 工程同学**（按业务路径调用，对齐契约）：`from evercore.user_memory import ChatMemCellExtractor`
- **算法同学**（按算法物理路径迭代 boundary 共享底层）：`from evercore.boundary.chat import ChatMemCellExtractor`

两条路径指向**同一个类**。物理路径与外部契约通过 `__init__.py` re-export 解耦。

### 4.1 物理目录速览（你打开 IDE 看到的样子）

monorepo + uv workspace 布局，每个 distribution 独立 `pyproject.toml`，源码在 `src/evercore/<subpackage>/`：

```
evercore/                                   # 仓库根（uv workspace）
├── pyproject.toml                          # workspace 根
├── uv.lock                                 # 整 workspace 共享 lockfile
├── packages/
│   ├── evercore-core/                      # 基础设施 dist
...
│   ├── evercore-boundary/  src/evercore/boundary/      # MemCell 切分（chat / workspace / agent）
│   ├── evercore-clustering/src/evercore/clustering/    # cluster_by_geometry / cluster_by_llm
│   ├── evercore-rank/      src/evercore/rank/          # 4 业务 facade + fusion / weight / rerank
│   ├── evercore-parser/    src/evercore/parser/        # image / audio / document / video / url
│   ├── evercore-user-memory/   src/evercore/user_memory/   # episode / foresight / atomic_fact / profile
│   ├── evercore-agent-memory/  src/evercore/agent_memory/  # case / skill
│   └── evercore-knowledge/     src/evercore/knowledge/     # extractor
├── docs/                                   # 你正在读的目录
├── tests/                                  # 跨包集成测试
└── local/                                  # 本地产物（git 忽略）
```
...
- 顶层 `evercore/` 是 [PEP 420](https://peps.python.org/pep-0420/) **隐式命名空间包**——**无 `__init__.py`**，多个 distribution 共享 `evercore.*` import 路径
- 子包 `evercore/<name>/` 是 regular package，**有 `__init__.py`** 承载 re-export
- `prompts` 在 `evercore-core/src/evercore/prompts/` 下；具体 prompt 字符串就近放各业务子包 `prompts/{en,zh}/<name>.py`（不外置 `.md` / `.yaml`）
...
| import 路径 | underscore + 共享 `evercore.*` | `evercore.user_memory` |
...
EverCore 算子用 **3 层 LLM 注入**（DSPy 同款），优先级 **per-call > scoped > default**：

| 层 | 用法 | 优先级 | 典型场景 |
|----|------|--------|---------|
| **per-call** | `aextract(..., llm=client)` 直接传参 | 最高 | EverOS 单调用按 scene 注入（**生产主路径**）|
| **scoped** | `async with evercore.llm.use(client):` | 次高 | EverOS pipeline 段批量同 client |
| **default** | 启动期 `evercore.configure(llm=...)` | 兜底 | dev / 测试 / Jupyter |
...
evercore.configure(llm=LLMConfig(
...
**EverCore 无 scene 概念**——"哪个算法步骤用哪个模型"是业务决策，由 EverOS 持有 `SceneRouter`，3 层注入按场景选用。EverCore 算子签名只接 `llm: LLMClient | None = None`，内部用 `evercore.llm.resolve(llm)` 一行解析三层 fallback。

> "为什么 scene 路由不在 EverCore" + "为什么 3 层注入" → [ADR 012](../decisions/012-llm-stack-architecture.md)

---

## 7. 状态管理边界

EverCore 是无状态算法库。任何"累积状态"都遵循同一函数式模式：

| 谁 | 职责 |
|---|------|
| **EverCore** | 定义"state 是什么"（值对象类型 + 字段 + 序列化）+ "state 怎么演化"（值对象方法返回新实例）|
| **caller**（EverOS 编排器）| "state 何时载入 / 存哪里 / 谁加锁"——持久化、并发控制、事务回滚 |

以 `ClusterState` 为例：

```python
from evercore.clustering import (
...
值对象 in / out（`@dataclass(frozen=True)`），EverCore 永不 mutate 入参——算法异常不污染原 state，事务安全。

不同业务的 state 实例物理隔离：user_memory 编排器持一个 `ClusterState` 实例装 episode 簇、agent_memory 编排器持另一个实例装 case 簇。**类型在 EverCore 单一定义**，**实例由 caller 各自持有**。
...
| 理解为什么 EverCore 无状态 | `concepts/stateless-design.md`（待补）|
...
| [003](../decisions/003-namespace-package-pep420.md) | PEP 420 共享 `evercore.*` namespace |
