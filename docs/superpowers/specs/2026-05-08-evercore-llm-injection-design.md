...
子项目 2 (LLM Stack) 设计 spec §3 line 277 把对外 surface 收敛到 7 个符号（`LLMClient` Protocol + `ChatMessage` / `ChatResponse` / `Usage` / `LLMConfig` / `LLMError` / `build_client`），其中只有 `build_client(config)` 是 LLM 实例的获取入口（per-call 注入主路径）。但 design.md line 826 / 833 / 841 明确写出 EverCore 算子内部的标准形态：
...
`evercore.llm.resolve(llm)` 对外是 1 行，但内部要做 3 层 fallback：

| 层 | 用法 | 优先级 | 典型场景 |
|----|------|--------|----------|
| **per-call** | `aextract(..., llm=client)` 直接传参 | 最高 | EverOS 单调用按 scene 注入（主路径）|
| **scoped** | `with evercore.llm.use(client):` contextmanager | 次高 | EverOS pipeline 段批量同 client（减少重复传参）|
...
EverCore 是算法库（vs 端到端框架），其公开 API 接受**已构造好的配置对象**（`LLMConfig` / `LLMClient`），但**不负责从外部源（env / TOML / yaml / k8s ConfigMap / secret manager / pydantic-settings ...）加载或解析配置**。design.md line 815-822 提到的 `EVERCORE_LLM_*` env vars 是**调用方惯例 hint**，不是 EverCore 自带 feature。本子项目仅落地接受配置的 5 个符号，不实现任何 loader。

详见 memory `feedback_evercore_accepts_config_not_loads.md`（2026-05-08 BOSS 拍板）。
...
```python
# evercore/llm/__init__.py（修正后完整版）

"""evercore.llm 子包 facade — 重新导出公开 surface + 注入机制 module-level 函数。
...
```python
# evercore/llm/errors.py（仅追加 1 个类）
...
- 5 个新公开符号在 `evercore.llm` 正确导出（`from evercore.llm import configure, use, current, resolve, LLMNotConfiguredError` 可用）
...
- 全部测试 PASS：`uv run pytest packages/evercore-core/ -v`（子项目 1+2+3 累计 ~114 + 子项目 2.5 新增 ~15 ≈ 129 tests）
...
**EverCore 取舍**：选 pydantic-ai 模式（B 选项）— `LLMNotConfiguredError(RuntimeError)`，与 LLMError SDK family 分家。详见 §5。
...
| 4 | env / TOML / yaml loader | **不实现** | BOSS Q4 拍板（2026-05-08）+ memory `feedback_evercore_accepts_config_not_loads.md` |
...
EverCore 算法库定位（per memory `feedback_evercore_accepts_config_not_loads.md`）。调用方有完全自由度：

```python
# 调用方惯例（不属 EverCore feature）
config = LLMConfig(
    provider="openai_compat",
    api_key=os.environ["EVERCORE_LLM_API_KEY"],   # 调用方自己读 env
    base_url=os.environ["EVERCORE_LLM_BASE_URL"],
    model=os.environ["EVERCORE_LLM_MODEL"],
)
client = build_client(config)
evercore.configure(llm=client)
```

design.md line 815-822 提到 `EVERCORE_LLM_*` env vars 是**调用方惯例 hint**，不是 EverCore 自带 feature。
...
业务编排归 EverOS（design.md §2.5 + ADR 012 line 168 明示）。EverCore 完全无 scene 概念。

### 10.6 不做 per-instance Agent override（pydantic-ai 风格）

EverCore 算子是 stateless（[ADR 011](docs/decisions/011-protocol-vs-abc.md) Protocol 而非 ABC），无 instance state 持有 client。pydantic-ai `agent.override(...)` 是 per-Agent-instance ContextVar，因为它是 stateful Agent 类。EverCore 选模块级 ContextVar 路径，per-call 已覆盖该需求。
