# EverCore Reference Implementation + CI 子项目设计文档
...
> 下游：本子项目落地后，EverCore 进入 release-ready 状态。
>
> 落盘日期：2026-05-08
> 设计依据：design.md §1.2 / §2.3 / §2.5 + ADR 010 + ADR 011 + memsys_opensource release/20260403 reference 算法。

**Goal**：在 design.md / ADR 既定框架内，落地 `evercore-boundary` + `evercore-user-memory` 的最小集 reference 算法（boundary 检测 + episode 提取），覆盖端到端 boundary→episode 数据流，并升级 `.gitlab-ci.yml` 到 strict 模式。
...
- `evercore-boundary` 是空骨架（仅 `pyproject.toml` + `README.md` + 空 `__init__.py`）
- `evercore-user-memory` 是空骨架同样
- 5 个产品性子包（agent_memory / knowledge / rank / parser / clustering）也都是空骨架

子项目 4 的目标是**最小可工作的 reference impl** —— 让 EverCore 算法库**接上就能用**。具体：
- 算法同学拿到 EverCore 后，能跑 `await EpisodeExtractor().aextract(memcell, llm=client)` 真实从 LLM 提取 Episode
...
**B 档已选**：算法骨架可工作，prompt 是真实可调优的精简版；EverCore 算法库定位下「算法同学接上就能调到 production」即够用，C 档（完整复现 opensource SOTA）不属算法库职责，应由 EverOS 编排层落地。
...
├── pyproject.toml          # MODIFY: 暂去 evercore-clustering 依赖（最小集不需要）
...
### 3.1 `evercore.boundary.chat.ChatMemCellExtractor`
        ...
        *,
        ...
        Args:
            ...
            prompt: Per-call prompt override. Defaults to
                ...
                ``evercore.boundary.prompts.zh.chat``.
...
> - **为什么这样设计**：ADR 010 line 205-212 是 EverCore 唯一推荐模式；ADR 011 line 117 选 Protocol structural（不强制继承）；class 提供 namespace + sync 桥接合并位置，private helpers (`_concat_messages` / `_format_messages_for_prompt` / `_build_memcells_from_llm_response` / `_make_memcell`) 单测友好（algorithm authors 可 monkeypatch 替换）
...
### 3.2 `evercore.boundary._tokenize.count_tokens`
...
### 3.3 `evercore.user_memory.episode.EpisodeExtractor`
...
> - **为什么这样设计**：design.md line 122 + line 311-313 已规定双访问路径：`from evercore.user_memory import ChatMemCellExtractor`（EverOS 契约）+ `from evercore.boundary.chat import ChatMemCellExtractor`（算法同学按算法族 import）
> - **规范依据**：design.md §1.2 line 75-78 "EverOS 文档契约 ... 通过各产品性子包的 __init__.py re-export 实现，与物理路径解耦"
> - **备选方案**：仅暴露 `evercore.boundary.chat` 单一路径 —— 否决，破坏 design.md 既定契约
...
```python
import evercore.boundary.chat as chat_mod
import evercore.boundary.prompts.zh.chat as zh
...
### 5.1 `evercore-boundary/tests/boundary/`（共约 8 测试）
...
- `test_chat_memcell_extractor_exported` —— `evercore.boundary.ChatMemCellExtractor` 可访问
- `test_dunder_all_lists_one_symbol` —— `__all__` 长度 1

### 5.2 `evercore-user-memory/tests/user_memory/`（共约 5 测试）
...
**`test_public_api.py`**（2 测试）：
- `test_chat_memcell_extractor_re_exported_from_boundary` —— `evercore.user_memory.ChatMemCellExtractor is evercore.boundary.ChatMemCellExtractor`（identity check 验证 re-export）
...
def reset_evercore_llm_state():
    """Reset evercore.llm._default + _active per test (sub-project 2.5)."""
    saved = evercore.llm._default
    token = evercore.llm._active.set(None)
    try:
        evercore.llm._default = None
        yield
    finally:
        evercore.llm._default = saved
...
| 测试文件 | 路径 |
|---|---|
| `test_chat.py` | `packages/evercore-boundary/tests/boundary/test_chat.py` |
| `test_tokenize.py` | `packages/evercore-boundary/tests/boundary/test_tokenize.py` |
| `test_public_api.py` | `packages/evercore-boundary/tests/boundary/test_public_api.py` ⚠ 同名冲突 |
| `test_episode.py` | `packages/evercore-user-memory/tests/user_memory/test_episode.py` |
| `test_public_api.py` | `packages/evercore-user-memory/tests/user_memory/test_public_api.py` ⚠ 同名冲突 |
...
| 改名后 | 路径 |
|---|---|
| `test_boundary_public_api.py` | `packages/evercore-boundary/tests/boundary/test_boundary_public_api.py` |
| `test_user_memory_public_api.py` | `packages/evercore-user-memory/tests/user_memory/test_user_memory_public_api.py` |
...
- `e2e` 测试用 autouse fixture 重置 `evercore.llm._default` + `_active`（与子项目 2.5 模式一致）
- boundary / user_memory unit tests 用 per-call `llm=fake` 注入，不依赖 `evercore.llm` 全局状态（无需 fixture）
...
- `build` stage —— 首次 PyPI release 再加（每个 `evercore-*` distribution 独立 build）
...
| 3 | LLM 注入 | per-call `llm: LLMClient \| None = None` + `evercore.llm.resolve(llm)` | 子项目 2.5（已落地）+ design.md line 833 |
...
| 8 | 错误层 | EverCore 不加 retry / fallback；LLM SDK 自带 2 次重试 | design.md §2.3 line 528-530 |
| 9 | `evercore-boundary/pyproject.toml` 加 `asgiref>=3.0` | sync 桥接需要 | ADR 010 line 220 |
| 10 | `evercore-user-memory/pyproject.toml` 暂去 `evercore-clustering` 依赖 | 最小集 EpisodeExtractor 不依赖 cluster | design.md §2.3 line 540 + line 686 |
...
EverCore 选 **stateless class + module-level helpers** 模式 —— 类似 sklearn `KMeans()` 但完全无 `fit()` / 状态持有；ADR 011 已选 Protocol structural（不强制继承），保留 BYOI（Bring Your Own Implementation）灵活性。
...
EverCore 简化版（no env-based selection，per-call + monkey-patch 双路径）。
...
EverCore 选**单一矩阵**（不分 package）—— scope 控制，子项目 5+ 真有 path-based 矩阵需求时升级。
...
| 2 | LLM 注入机制 | 子项目 2.5 `evercore.llm.resolve()` | 子项目 2.5 已落地 |
...
5. `clustering/*` —— 独立子包 `evercore-clustering`，子项目 6+
6. `rank/*` —— 独立子包 `evercore-rank`，子项目 7+
