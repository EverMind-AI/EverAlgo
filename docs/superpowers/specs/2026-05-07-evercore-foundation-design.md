# EverCore Foundation 子项目设计文档

| 字段 | 值 |
|------|-----|
| 子项目编号 | 1（共 4 子项目） |
| 状态 | Brainstorm 完成（已收紧到 EPISODE 最小集），**待 BOSS 审 spec**；通过后 → writing-plans |
| 范围 | (a) `evercore.types.*` —— **EPISODE 路径最小集** 4 个对外符号 + (b) `evercore.prompts.validator` 2 函数 |
...
### 2.3 `evercore.types.__init__` re-export 清单

```python
"""Public data contracts for EverCore — minimal EPISODE-path subset."""

from evercore.types.memcell import Message, MessageRole, MemCell
...
→ 4 个对外符号。算法同学可 `from evercore.types import MemCell` 或 `from evercore.types.memcell import MemCell` 两条路径。

---

## 3. Prompts validator

### 3.1 `evercore.prompts.validator.check_placeholders`
...
### 3.2 `evercore.prompts.validator.check_length`
...
| 跨包 fixture | 通过 `evercore.testing.*` 公开 import | tests 不安装到 venv，跨包 import 唯一可靠路径 |
...
### 5.1 `evercore-core/pyproject.toml` 改动
...
2. ✅ `uv run python -c "from evercore.types import MemCell, Message, MessageRole, Episode; from evercore.prompts.validator import check_placeholders, check_length; print('OK')"` 输出 `OK`
3. ✅ `uv run pytest packages/evercore-core/tests/` 全绿
...
- **`ChatMessage / ChatResponse / Usage / ChatChunk`** —— LLM stack 子项目（归 `evercore.llm.types` 内部，与 `evercore.types` 区分）

### 7.3 架构改动暂不做

- **ADR 013（ClusterState 上提到 `evercore.types.cluster`）** —— 没 ClusterState type，物理位置改动不需要
- **`docs/design.md` §2.4 line 564 注释 patch** —— 同上
- **`evercore.types.common.py` / `agent.py` / `cluster.py` 三个文件** —— 没对应 type，不创建空文件
