# EverCore LLM 3-Layer Injection Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 `evercore-core` 子项目 2 已落地的 `evercore.llm` 子包内增量补 5 个新公开符号（`LLMNotConfiguredError` / `configure` / `use` / `current` / `resolve`）+ 2 个 module-level 私有 state（`_default` / `_active` ContextVar），让算子内部用 `evercore.llm.resolve(llm)` 单行解析 3 层 fallback（per-call > scoped > default）。

**Architecture:** 4 个 module-level 函数 + 2 个私有 state 全部放在 `evercore/llm/__init__.py`（design.md line 786 既定 facade 结构）；新增错误类 `LLMNotConfiguredError(RuntimeError)` 加进 `errors.py`（与 LLMError SDK family 分家）。`use()` 是 sync `@contextmanager`，底层 `contextvars.ContextVar` 自动 async-safe（FastAPI / asyncio 7 场景验证）。**不**新建源文件、**不**做 env/TOML loader、**不**支持 reset。
...
3. **`mypy` 必须从仓库根（`/Users/admin/Documents/evermemos/evercore`）跑**，**不要**从 `packages/evercore-core/` 子目录跑（mypy_path 配置在根 `pyproject.toml`，子目录跑会触发误报 `import-untyped`）。
...
```
packages/evercore-core/src/evercore/llm/
...
packages/evercore-core/tests/llm/
...
设计依据：`docs/superpowers/specs/2026-05-08-evercore-llm-injection-design.md` §2 (File Map) 和 §3 (公开 API)。
...
**Files:**
- Modify: `packages/evercore-core/src/evercore/llm/errors.py`
- Modify: `packages/evercore-core/tests/llm/test_errors.py`

- [ ] **Step 1: Append failing test to `test_errors.py`**

读取现有 `packages/evercore-core/tests/llm/test_errors.py` 的内容，在文件末尾追加：
...
注意：`LLMError` 在文件顶部应该已经 imported（子项目 2 已有）；如果没有，加 `from evercore.llm.errors import LLMError` 到 import 块。

- [ ] **Step 2: Run test to verify it fails**

```bash
uv run pytest packages/evercore-core/tests/llm/test_errors.py -v
```

Expected: FAIL with `ImportError: cannot import name 'LLMNotConfiguredError' from 'evercore.llm.errors'`.

- [ ] **Step 3: Append `LLMNotConfiguredError` to `errors.py`**

读取现有 `packages/evercore-core/src/evercore/llm/errors.py`，在 `LLMError` 类定义之后追加：
...
```bash
git add packages/evercore-core/src/evercore/llm/errors.py \
        packages/evercore-core/tests/llm/test_errors.py
...
**Files:**
- Modify: `packages/evercore-core/src/evercore/llm/__init__.py`
- Create: `packages/evercore-core/tests/llm/test_injection.py`

- [ ] **Step 1: Create `test_injection.py` with fixture + first test**

写入 `packages/evercore-core/tests/llm/test_injection.py`:
...
```bash
uv run pytest packages/evercore-core/tests/llm/test_injection.py -v
```

Expected: FAIL with `AttributeError: module 'evercore.llm' has no attribute '_default'` (or similar — `configure` / `_active` 也不存在).

- [ ] **Step 3: Modify `__init__.py` to add module state + `configure`**

读取现有 `packages/evercore-core/src/evercore/llm/__init__.py`，在文件顶部 import 块之后、现有 re-export 之后追加（**保留**所有子项目 2 既有 import 和 `__all__`）：
...
``evercore.llm._default = ...`` for isolation, but this is not part of
the documented public API.
"""

_active: contextvars.ContextVar[LLMClient | None] = contextvars.ContextVar(
    "evercore_llm_active", default=None,
)
...
```bash
git add packages/evercore-core/src/evercore/llm/__init__.py \
        packages/evercore-core/tests/llm/test_injection.py
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `packages/evercore-core/src/evercore/llm/__init__.py`
- Modify: `packages/evercore-core/tests/llm/test_injection.py`
...
Expected: 4 new tests FAIL with `AttributeError: module 'evercore.llm' has no attribute 'use'`.
...
Expected: 3 new tests FAIL with `AttributeError: module 'evercore.llm' has no attribute 'current'`.
...
Expected: 5 new tests FAIL with `AttributeError: module 'evercore.llm' has no attribute 'resolve'`.
...
def resolve(per_call: LLMClient | None = None) -> LLMClient:
    ...
    Args:
        per_call: Per-call override passed by the operator's caller.
            ...
            then internally ``client = evercore.llm.resolve(llm)``.
    ...
    if client is None:
        raise LLMNotConfiguredError(
            ...
            "wrap in `evercore.llm.use(client)`, "
            "or call `evercore.configure(llm=client)` at startup."
        )
    return client
```

注意：`LLMNotConfiguredError` 必须 import 进来。在文件顶部 `from evercore.llm.errors import LLMError` 一行旁边追加（合并为单 import）：

```python
from evercore.llm.errors import LLMError, LLMNotConfiguredError
```
...
client = evercore.llm.resolve(llm) 一行解析 3 层 fallback：
...
**Files:**
- Modify: `packages/evercore-core/src/evercore/llm/__init__.py`
- Modify: `packages/evercore-core/tests/llm/test_public_api.py`

- [ ] **Step 1: Update `test_public_api.py`**

读取现有 `packages/evercore-core/tests/llm/test_public_api.py`。子项目 2 的测试断言 `__all__` 长度为 7（具体测试函数名可能是 `test_dunder_all_lists_exactly_7_symbols` 或类似）。**修改**这个测试：
...
def test_dunder_all_lists_exactly_7_symbols() -> None:
    ...
    assert sorted(evercore.llm.__all__) == sorted([
        "ChatMessage",
        "ChatResponse",
        "LLMClient",
        "LLMConfig",
...
```bash
uv run pytest packages/evercore-core/tests/llm/test_public_api.py -v
```
...
读取现有 `packages/evercore-core/src/evercore/llm/__init__.py`。找到 `__all__ = [...]` 块，把 5 个新符号加入（保持 alphabetical）：
...
```bash
uv run pytest packages/evercore-core/tests/llm/ -v
```
...
- pytest: **整个 evercore-core 累计 ~129 tests PASS**（子项目 1+2+3 的 ~114 + 子项目 2.5 新增 ~15）
...
```bash
git add packages/evercore-core/src/evercore/llm/__init__.py \
        packages/evercore-core/tests/llm/test_public_api.py
git commit -m "$(cat <<'EOF'
✨ feat(llm): expose 5 new symbols at evercore.llm top level (3-layer injection)
...
- Ready for sub-project 4 (reference impl) — operators can now use the
  spec line 833 form: `client = evercore.llm.resolve(llm)`.
...
| §6.4 测试 fixture 隔离 | Task 2 创建 `reset_evercore_llm_state` autouse fixture |
...
`grep -nE "TODO|TBD|FIXME|XXX|implement later|fill in" docs/superpowers/plans/2026-05-08-evercore-llm-injection.md`
预期：0 hits。

### 3. Type consistency

- `LLMClient` 类型在 Task 2-6 全部一致（来自 `evercore.llm.protocols`）
