"""English prompts for EpisodeExtractor — verbatim port from new-release opensource.

Source: ``opensource/evermemos-opensource/src/memory_layer/prompts/en/episode_mem_prompts.py``.

Constants:
    - ``DEFAULT_CUSTOM_INSTRUCTIONS`` — block of instructions injected into the {custom_instructions} slot.
    - ``EPISODE_GENERATION_PROMPT`` — personal mode.
    - ``GROUP_EPISODE_GENERATION_PROMPT`` — group mode.

Placeholders: ``{conversation_start_time}`` / ``{conversation}`` / ``{custom_instructions}``. Rendered
via :py:meth:`str.format`.
Output schema: ``{"title": str, "content": str}``.
"""
