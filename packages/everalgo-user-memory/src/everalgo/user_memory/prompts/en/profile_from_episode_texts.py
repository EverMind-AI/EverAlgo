...
_SOURCE_RULES = """\
The source records are third-person Episode narratives rather than raw conversation turns. In the profile rules below, "conversation" means the supplied Episode narratives. Each narrative opens with its observation date on its own line, written as [YYYY-MM-DD HH:MM:SS UTC]: that is when the narrated conversation took place, not when this extraction runs, and the narratives are given oldest first. Treat a fact as stated only when a narrative explicitly attributes that fact, preference or constraint to {target_user}; a quotation mark is not required. An action alone remains an event, not an explicit_info fact."""
