...
重要时间处理规则：
- 将提供的"对话开始时间"作为本次对话/情节的精确起始时间
- 当对话中出现相对时间（如"昨天""上周"）时，必须同时保留原始相对表述并计算出对应的绝对日期
- 时间引用格式："原始相对表达（绝对日期）"——例如"上周（2023-05-07）"
- 这种双重格式同时支持基于绝对时间和相对时间的提问
- 所有绝对时间的计算都必须基于提供的对话开始时间

请生成结构化情节记忆，仅返回包含以下两个字段的 JSON 对象：
{{
    "title": "一段精炼、描述性的标题，准确概括主题（10-20 个词）",
    "content": "以第三人称叙述的精炼客观对话记录。必须涵盖所有重要信息：谁、何时参与、讨论了什么、做出了什么决策、表达了什么情绪、形成了什么计划或结果。按时间顺序组织，聚焦可观察的行为和直接陈述。删除冗余表达和啰嗦描述，同时保留所有事实、实体（姓名、日期、地点）和具体细节。保持精炼但不丢失关键信息。使用提供的对话开始时间作为本情节的基准时间。"
}}
...
7. 时间引用使用双重格式："相对时间（绝对日期）"以支持不同类型的问题。
...
8.  **时间引用——双重格式**：
    - 当对话中出现相对时间（如"昨天""上周"）时，必须同时保留原始相对表述并计算出对应的绝对日期
    - 格式："原始相对表达（绝对日期）"——例如"上周（2023-05-07）"
    - 所有绝对时间的计算都必须基于提供的对话开始时间
