...
- Episode bodies now carry a code-built UTC timestamp prefix (`2026-05-29 12:25 UTC — …`) instead of one written by the LLM. The model used to translate the injected English anchor into the output language, which produced drifting formats and, in the worst case, mistranslated `12:21 PM` into a Chinese "before noon" phrasing. Timestamps injected into the prompt switched to a 24-hour format (`2026-05-29 12:25 UTC (Friday)`); the weekday label is retained because relative-time resolution depends on it.
- Episode prompts: all date examples unified on `YYYY-MM-DD`.
- Episode prompts now require every absolute time that states a clock time to carry the `UTC` zone label (`2024-03-14 15:00 UTC`, never `2024-03-14 15:00` or a bare `15:00`), in all four variants (generic and user-centred × `en` and `zh`). A real-LLM run over mixed-language conversations wrote the narrative's per-event times with the label 25 times and without it 15 times, which is the same "sometimes UTC, sometimes not" inconsistency the code-built prefix fixed for the body's opening but left untouched inside the narrative. Dates without a clock time need no label, and clock times quoted from a speaker keep their original wording (`at 3:30 PM`) because the speaker's timezone is unknown.
- An instruction added earlier on this branch — forbidding `content` from opening with a date or time of any form — was removed. It contradicted the same prompt's demand for a chronological account carrying per-event times, and the model ignored it in all 15 attempts of a real-LLM run. Correctly so: those times belong to the events, while the code-built prefix is metadata about the slice. The two state the same moment whenever the first event is the conversation's opening message; that overlap is expected and is documented as such in both prompt modules and in `aextract`'s `prompt=` contract.
- `EpisodeReflector`'s rendered timeline no longer brackets each episode with a `format_natural_language_time` timestamp: that value was `Episode.timestamp` (closing time) in the 12-hour format this branch removed from episode prompts, and it disagreed with the episode body's own code-built prefix (span-start time). The bracket is dropped; the body's prefix is now the sole time source in the timeline.
- `EpisodeReflector`'s merged `Episode` now carries the same code-built UTC timestamp prefix as `EpisodeExtractor` output (`_build_episode`). The prefix is derived from `episodes[0].timestamp` (span start); `Episode.timestamp` still holds `episodes[-1].timestamp` (span end) as before. Previously the merged body carried no prefix at all, silently breaking the "every episode body carries a prefix" invariant whenever reflection ran.
- `EpisodeExtractor` now rejects an empty or whitespace-only `content` field from the LLM with a `ValueError` instead of silently accepting it. Previously, once the timestamp prefix was prepended, an empty `content` produced a technically non-empty body (just the prefix), which defeated both `assert_episode_shape`'s and the benchmark extract stage's empty-body guards.
