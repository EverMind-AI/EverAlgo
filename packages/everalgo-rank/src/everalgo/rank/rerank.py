...
FusionMode = Literal["rrf", "lr", "mrag", "vector_anchored"]
...
async def _basic_arank(  # noqa: C901
    ...
    *,
    ...
    if config.fusion_mode == "mrag":
        ...
        # Wrap pre-fetched lists as RetrieveFn callables for ahierarchical_retrieve.
        _sparse_list = list(sparse)
        _dense_list = list(dense)
        _fact_map = rank_input.episode_to_facts

        async def _dense_retrieve(q: str, k: int) -> list[Candidate]:
            return _dense_list

        async def _sparse_retrieve(q: str, k: int) -> list[Candidate]:
            return _sparse_list

        async def _child_retrieve(q: str, parent: Candidate, k: int) -> list[Candidate]:
            facts = _fact_map.get(parent.id, [])
            return [
                Candidate(
                    id=f.id,
                    score=f.score,
        ...
        results = await ahierarchical_retrieve(
            ...
            parent_dense_retrieve=_dense_retrieve,
            parent_sparse_retrieve=_sparse_retrieve,
            child_retrieve_for_parent=_child_retrieve,
            response_top_k=rank_input.top_k,
            rrf_k=config.rrf_k,
            expand_limit=config.expand_limit,
            children_per_parent=config.expand_limit,
            alpha=config.alpha,
            max_convergence_rounds=config.max_convergence_rounds,
        )
        scored = []
        for c in results:
            source_kind = c.metadata.get("source_kind", "parent")
            if source_kind == "child":
                parent_ep_id = c.metadata.get("parent_id", "")
                scored.append(
                    ScoredItem(
                        id=c.id,
                        score=c.score,
                        item_type="atomic_fact",
                        parent_episode_id=parent_ep_id if isinstance(parent_ep_id, str) else None,
                        metadata={k: v for k, v in c.metadata.items() if k not in ("source_kind", "parent_id")},
                    )
                )
            else:
                scored.append(
        ...
        meta: dict[str, Any] = {"stage": item_type, "fusion_mode": "mrag"}
    else:
