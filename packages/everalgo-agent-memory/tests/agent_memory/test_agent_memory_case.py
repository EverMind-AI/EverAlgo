...
Port of opensource ``tests/test_agent_case_extractor.py`` algorithm-IP tests, retargeted to:

- :class:`everalgo.types.Message` / :class:`everalgo.types.ToolCall` instead of raw OpenAI dicts
...
After the case.py Message-native refactor, all private pipeline helpers operate directly on
:class:`Message` objects; OpenAI-format dict conversion happens only at LLM-prompt boundaries via
:func:`_to_openai_dicts` / :func:`_dump_messages`.
"""
...
from everalgo.types import MemCell, Message, MessageRole, ToolCall

# ── Fixture helpers — typed Message instances throughout ─────────────────────────────────────────────
...
def _user_msg(content: str, ts: int = 1700000000000) -> Message:
    return Message(role=MessageRole.USER, content=content, timestamp=ts)


def _assistant_msg(content: str, ts: int = 1700000001000) -> Message:
...
def _tool_call_msg(
    ...
    return Message(
        ...
        content=content,
        timestamp=ts,
        tool_calls=[ToolCall(id=call_id, function={"name": name, "arguments": arguments})],
    )


def _tool_response_msg(content: str = "result", call_id: str = "call_1", ts: int = 1700000000600) -> Message:
    return Message(role=MessageRole.TOOL, content=content, timestamp=ts, tool_call_id=call_id)
...
class TestShouldSkip:
    ...
    def test_incomplete_last_is_tool_call(self) -> None:
        msgs = [_user_msg("do X"), _tool_call_msg("thinking")]
        assert "Incomplete" in (_should_skip(msgs) or "")

    def test_incomplete_last_is_tool_response(self) -> None:
        msgs = [_user_msg("do X"), _tool_call_msg(), _tool_response_msg()]
...
class TestStripBeforeFirstUser:
    ...
    def test_drops_leading_non_user(self) -> None:
        """Leading assistant/tool messages (e.g. system framing relayed as assistant) are dropped."""
        msgs = [_assistant_msg("setup"), _user_msg("hi"), _assistant_msg("hello")]
        result = _strip_before_first_user(msgs)
        assert len(result) == 2
        assert result[0].role == MessageRole.USER

    def test_no_user_returns_empty(self) -> None:
        msgs = [_assistant_msg("hi")]
...
class TestToolCallHelpers:
    def test_has_tool_calls_via_assistant(self) -> None:
        msgs = [_user_msg("x"), _tool_call_msg(), _tool_response_msg(), _assistant_msg("done")]
        assert _has_tool_calls(msgs) is True

    def test_has_tool_calls_via_tool_response_only(self) -> None:
        msgs = [_user_msg("x"), _tool_response_msg("ok"), _assistant_msg("done")]
        assert _has_tool_calls(msgs) is True
...
class TestCalcToolContentSize:
    ...
    def test_multiple_tool_calls_sum(self) -> None:
        msg = Message(
            ...
            timestamp=1,
            tool_calls=[
                ToolCall(id="c1", function={"name": "f1", "arguments": '{"a": 1}'}),
                ToolCall(id="c2", function={"name": "f2", "arguments": '{"b": 2}'}),
            ],
        )
...
class TestCollectToolCallGroups:
    def test_single_group(self) -> None:
        items = [_user_msg("hi"), _tool_call_msg(), _tool_response_msg(), _assistant_msg("done")]
    ...
    def test_tool_call_without_response(self) -> None:
        items = [_user_msg("hi"), _tool_call_msg(), _assistant_msg("done")]
...
class TestCalcGroupSize:
    def test_group_size_is_sum(self) -> None:
        items = [_tool_call_msg(arguments='{"query": "test search"}'), _tool_response_msg("tool result here")]
...
class TestHeuristicTrim:
    def test_short_content_unchanged(self) -> None:
        ...
        assert trimmed[2].content == "short result"
    ...
    def test_long_tool_output_trimmed_when_scale_kicks_in(self) -> None:
        ...
        assert "[... trimmed" in trimmed[2].content
        assert len(trimmed[2].content) < len(long)
...
class TestToOpenaiDicts:
    def test_basic_message_serialisation(self) -> None:
        ...
        assert isinstance(dicts[0]["role"], str)  # MessageRole.value, not enum

    def test_assistant_with_tool_calls_drops_none_content(self) -> None:
        msgs = [_tool_call_msg(content=None, arguments='{"q": "x"}')]
        dicts = _to_openai_dicts(msgs)
...
class TestIsWorthExtracting:
    ...
    async def test_malformed_json_defaults_to_true(self) -> None:
        """Per opensource opinionated fallback: filter failure → assume worth extracting."""
...
def _od(msgs: list[Message]) -> list[dict[str, Any]]:
    """Wrap typed Message objects in the opensource ``{"message": ...}`` envelope shape."""
    return [{"message": m.model_dump(exclude_none=True)} for m in msgs]


class TestAgentCaseExtractorAExtract:
    @staticmethod
    def _memcell(messages: list[Message]) -> MemCell:
        return MemCell(event_id="mc_test", original_data=_od(messages), timestamp=1700000000000)

    async def test_empty_messages_returns_empty_list(self) -> None:
        fake = FakeLLMClient(responses=[])  # no LLM call expected
        cases = await AgentCaseExtractor().aextract(self._memcell([]), llm=fake)
        assert cases == []
    ...
    async def test_single_turn_no_tool_skipped_before_llm(self) -> None:
        ...
        msgs = [_user_msg("hi"), _assistant_msg("hello")]
        cases = await AgentCaseExtractor().aextract(self._memcell(msgs), llm=fake)
        assert cases == []
    ...
    async def test_multi_round_tools_skip_filter_go_to_compress(self) -> None:
        ...
        # parent linkage points to source memcell
        assert case.parent_type == "memcell"
        assert case.parent_id == "mc_test"
    ...
    async def test_compress_empty_task_intent_yields_no_case(self) -> None:
        """Per opensource skip semantics: empty task_intent → return [] regardless."""
    ...
    async def test_per_call_prompt_compress_override(self) -> None:
        ...
        await AgentCaseExtractor().aextract(self._memcell(msgs), llm=fake, prompt_compress=custom_prompt)
    ...
    async def test_per_call_prompt_filter_override(self) -> None:
        ...
        await AgentCaseExtractor().aextract(self._memcell(msgs), llm=fake, prompt_filter=custom_filter)
    ...
    async def test_monkey_patch_prompt_override_still_works(self) -> None:
        ...
        try:
            ...
            await AgentCaseExtractor().aextract(self._memcell(msgs), llm=fake)
...
class TestApplyTruncationToolCallEdgeCases:
    def test_tool_call_with_empty_arguments_skipped(self) -> None:
        ...
        msgs = [
            Message(
                ...
                content=None,
                timestamp=1,
                tool_calls=[ToolCall(id="call_1", function={"name": "f", "arguments": ""})],
            ),
        ]
    ...
    def test_tool_call_with_empty_function_dict_skipped(self) -> None:
        """Empty ``function`` dict → ``function.get("arguments", "")`` is "" → skipped without error."""
        msgs = [
            Message(
                role=MessageRole.ASSISTANT,
                content=None,
                ...
                tool_calls=[
                    ...
                    ToolCall(id="call_2", function={"name": "x", "arguments": "a" * 5000}),
                ],
            ),
        ]
        result = _apply_truncation(msgs, max_tool_output=1000, max_tool_args=100, max_assistant=3000)
        assert result[0].tool_calls is not None
        assert result[0].tool_calls[0].function == {}  # untouched
        assert len(result[0].tool_calls[1].function["arguments"]) < 5000  # trimmed
...
class TestPreCompressToList:
    ...
    async def test_oversized_triggers_selective_compression(self) -> None:
        """Two large groups + sum > chunk size → at least one gets selectively compressed via LLM."""
    ...
    async def test_compression_raises_falls_back_to_originals(self) -> None:
        """LLM raises during chunk compression → asyncio.gather captures it; originals kept."""
        ...
        big_responses_kept = sum(1 for m in result if m.role == MessageRole.TOOL and len(str(m.content or "")) > 1000)
        assert big_responses_kept == 2

    async def test_compression_returns_none_falls_back_to_originals(self) -> None:
        """LLM returns malformed JSON → _compress_tool_chunk returns None → fallback to originals."""
    ...
    async def test_compressed_count_mismatch_keeps_originals(self) -> None:
        """LLM returns a list of wrong length → _compress_tool_chunk returns None → fallback."""
...
class TestCompressToolChunk:
    ...
    async def test_json_decode_failure_returns_none(self) -> None:
        msgs = [_tool_response_msg("x")]
    ...
    async def test_compressed_list_wrong_length_returns_none(self) -> None:
        ...
        msgs = [_tool_call_msg(), _tool_response_msg(), _tool_response_msg(call_id="call_2")]  # 3 input msgs
        payload = json.dumps(
            {
                "compressed_messages": [
    ...
    async def test_compressed_message_validation_failure_returns_none(self) -> None:
        """A compressed message with an invalid role → Message.model_validate raises → None."""
        msgs = [_tool_response_msg("x")]
        payload = json.dumps({"compressed_messages": [{"role": "nonsense", "content": "x"}]})
        fake = FakeLLMClient(responses=[ChatResponse(content=payload, model="fake")])
        assert await _compress_tool_chunk(msgs, fake) is None
...
class TestAgentCaseExtractorTruncationLogging:
    async def test_task_intent_truncation_logged(self) -> None:
        ...
        cases = await AgentCaseExtractor().aextract(
            MemCell(event_id="mc_trunc", original_data=_od(msgs), timestamp=1700000000000), llm=fake
        )
        assert len(cases) == 1
...
class TestAgentCaseExtractorBetweenThresholds:
    async def test_total_between_chunk_size_and_2x_does_not_bail(self) -> None:
        ...
        fake = FakeLLMClient(responses=[ChatResponse(content=compress_response, model="fake")])
        cases = await AgentCaseExtractor().aextract(
            MemCell(event_id="mc_mid", original_data=_od(msgs), timestamp=1700000000000), llm=fake
        )
        assert len(cases) == 1
        assert fake.call_count >= 1
...
class TestAgentCaseExtractorPostTrimBail:
    async def test_extremely_oversized_trajectory_bails_after_trim(self) -> None:
        ...
        cases = await AgentCaseExtractor().aextract(
            MemCell(event_id="mc_huge", original_data=_od(msgs), timestamp=1700000000000), llm=fake
        )
        assert cases == []
        assert fake.call_count == 0
