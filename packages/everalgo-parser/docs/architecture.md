...
`from everalgo.parser import ...` 一处导入,5 个函数 + 3 个类型 + 5 个子模块。
...
| `image`, `audio`, `document`, `url`, `video` | 子模块 | 每个都暴露各自的 `aparse` / `parse` |
| `RawFile`, `ParsedContent`, `Modality` | type re-export | 来自 `everalgo.types` |

每个子模块的具体 `aparse` 签名跟顶层一致(`raw_file: RawFile, *, llm`),不重复列。
...
| 文件 | 数量 | 类型 | LLM |
|---|---|---|---|
| `test_public_api.py` | 3 | 公开 API 形状 | 不用 |
| `test_dispatch.py` | 8 | 顶层派发 | `FakeLLMClient` |
| `test_document.py` | ~7 | PDF / HTML / EML 单测 | `FakeLLMClient` |
| `test_url.py` | 8 | OG 抽取 + httpx mock | `FakeLLMClient` + `respx` |
| `tests/types/test_modality.py` | ~10 | Modality enum + get_modality | 不用 |
| `tests/types/test_parsed.py` | ~5 | ParsedContent 字段 | 不用 |
| `test_e2e_all_formats.py` | **16** | **真 OpenRouter / Gemini** | 真 LLM(env `OPENROUTER_API_KEY`)|

**单测总计**:`237/237`(parser+core)/ `689/689`(workspace 全量)
**E2E**:`16/16`,真模型,~1 分钟跑完
